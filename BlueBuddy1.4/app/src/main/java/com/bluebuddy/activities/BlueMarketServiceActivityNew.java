...
public class BlueMarketServiceActivityNew extends BuddyActivity implements LocationListener, GoogleApiClient.OnConnectionFailedListener {
    ...
    List<AllServiceDetails> allServiceDetailsList;
...
/*        TabsPagerAdapter pagerAdapter = new TabsPagerAdapter(getSupportFragmentManager());
        pagerAdapter.addFragment(AllServiceListFragment.createInstance(category, token, curru), "Services");
        pagerAdapter.addFragment(LocationSearchFragmentService.createInstance(category, token), "Location");
    ...
    void initialize_all() {
       ...
       /* frm_date = (Button) findViewById(R.id.frm_date);
        ...
        allServiceDetailsList = new ArrayList<AllServiceDetails>();
        serviceOfferedAdapter = new ServiceOfferedAdapter(_activity, _context, allServiceDetailsList, token, curru, category);
        ...
        fab.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                // Click action
                Intent intent = new Intent(_activity, ActivityAddYourServices.class);
...
//        checkBeforeServiceCall();
    ...
    private void currentlocationsearch() {
        int permission = ContextCompat.checkSelfPermission(BlueMarketServiceActivityNew.this, Manifest.permission.ACCESS_FINE_LOCATION);
        if (permission != PackageManager.PERMISSION_GRANTED) {
            Log.i("HELLO", "Permission to location denied");

            if (ActivityCompat.shouldShowRequestPermissionRationale(BlueMarketServiceActivityNew.this, Manifest.permission.ACCESS_FINE_LOCATION)) {
                AlertDialog.Builder builder = new AlertDialog.Builder(BlueMarketServiceActivityNew.this);
                ...
                builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int id) {
                        ...
                        ActivityCompat.requestPermissions(BlueMarketServiceActivityNew.this, new String[]{Manifest.permission.ACCESS_FINE_LOCATION, Manifest.permission.CAMERA, Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.WRITE_EXTERNAL_STORAGE}, RECORD_REQUEST_CODE);
                    }
                });
...
//        Intent i = new Intent(BlueMarketServiceActivityNew.this, Categories_bluemarketActivity.class);
//        startActivity(i);
        finish();
    }
    ...
    public void serverCall(String sendCategory, String user_lat, String user_long, String radius_send) {
        if (isActivityVisible()) {
            ...
            ApiInterface service = ApiClient.getClient().create(ApiInterface.class);
            //Call<AllService> userCall = service.allService("Bearer " + token);
            Call<AllService> userCall = service.allServiceSearch("Bearer " + token, user_lat, user_long, radius_send);
            //Call<AllTrip> userCall = service.allTripSearch("Bearer "+token,sendCategory,user_lat,user_long,radius_send,fromDate,toDate);
            userCall.enqueue(new Callback<AllService>() {
                ...
                public void onResponse(Call<AllService> call, Response<AllService> response) {
                    ...
                    if (response.code() == AppConfig.SUCCESS_STATUS) {
                        if (response.body().getStatus() == true && response.body().getService_list().size() > 0) {

                            response.body().getService_list().toString();

                            tv_noDataFound.setVisibility(View.GONE);
                            recyclerView.setVisibility(View.VISIBLE);
                            allServiceDetailsList.clear();
                            allServiceDetailsList.addAll(response.body().getService_list());
